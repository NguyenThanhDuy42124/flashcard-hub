...
const ExamCreator = () => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <div className="inline-flex items-center gap-2 rounded-full bg-blue-100 px-4 py-1.5 mb-4 text-sm font-semibold text-gray-700">
          <span>Trình tạo đề</span>
          <span>•</span>
          <span>{deckType}</span>
        </div>
        <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-2 tracking-tight">📝 Tạo đề thi</h1>
        <p className="text-gray-600 text-base md:text-lg">Chọn deck, phân bổ tỉ lệ và tạo đề trong một lần thao tác.</p>
      </div>

      <div className="flex flex-wrap items-center gap-3 bg-white rounded-2xl px-4 py-3">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {availableDecks.map((deck) => (
              <div key={deck.id} className="flex items-center justify-between py-3">
                ...
                <button
                  ...
                  className="px-3 py-1.5 rounded-xl text-sm font-semibold bg-blue-600 text-white hover:bg-blue-700"
                >
                  Thêm
                </button>
              </div>
        ...
        <div className="bg-white rounded-2xl p-5 space-y-4">
          ...
          {selectedDecks.length === 0 ? (
            ...
            <div className="space-y-3">
              {selectedDecks.map((deck) => (
                <div key={deck.id} className="p-4 border border-gray-200 rounded-xl flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      className="w-24 px-3 py-2 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500"
                    />
          ...
          {percentSum < 100 && selectedDecks.length > 0 && (
            <div className="text-sm text-orange-600 bg-orange-50 border border-orange-200 rounded-xl px-3 py-2 font-semibold">
      ...
      <div className="bg-white rounded-2xl p-5 space-y-4">
        ...
        <div className="pt-2">
          <button
            ...
            disabled={submitting}
            className={`w-full md:w-auto px-5 py-3 rounded-xl text-white font-semibold shadow ${
